...
4. Inside the extracted folder, rename the file `cre_v1.0.11_windows_amd64.exe` to `cre.exe`.
...
<Aside type="tip" title="Understanding trigger-index">
...
<Aside type="tip" title="Understanding the two different indexes">
...
const initWorkflow = (config: Config) => {
  ...
  return [handler(cron.trigger({ schedule: "0 */5 * * * *" }), onCronTrigger)]
}
...
<Aside type="tip" title="Choose the style that fits your codebase">
...
<Aside type="caution" title="No console access in workflows">
...
<Aside type="tip" title="When to use custom error handling">
...
| BNB Chain Testnet   | binance_smart_chain-testnet         | 13264668187771770619 |
