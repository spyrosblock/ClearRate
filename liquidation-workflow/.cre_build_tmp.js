...
var configSchema = exports_external.object({
  ...
  liquidationMonitoring: exports_external.object({
    apiEndpoint: exports_external.string()
  }).optional()
});
var liquidationTargetsResponseSchema = exports_external.object({
  success: exports_external.boolean(),
  liquidationTargets: exports_external.array(exports_external.object({
...
var fetchLiquidationTargets = (runtime2, sendRequester, config) => {
  ...
  runtime2.log(`Found ${targetCount} liquidation targets`);
  return parsedData;
};
var writeLiquidationReport = (runtime2, evmConfig, targets) => {
  ...
  runtime2.log(`Processing ${targets.length} liquidation targets on LiquidationEngine at ${evmConfig.liquidationEngineAddress}`);
  ...
  runtime2.log(`Encoded liquidation report data: ${reportData}`);
  ...
  runtime2.log(`Liquidation report submitted successfully! TxHash: ${txHash}`);
  return txHash;
};
var executeLiquidationWorkflow = (runtime2) => {
  runtime2.log("=== Liquidation Workflow Started ===");
  const evmConfig = runtime2.config.evms[0];
  const httpCapability = new ClientCapability2;
  runtime2.log("Fetching liquidation targets from monitoring API...");
  let targetsResponse;
  targetsResponse = httpCapability.sendRequest(runtime2, (sendRequester, config) => fetchLiquidationTargets(runtime2, sendRequester, config), ConsensusAggregationByFields({
    success: median,
    liquidationTargets: median,
    count: median
  }))(runtime2.config).result();
  const targets = targetsResponse.liquidationTargets || [];
  runtime2.log(`Fetched ${targets.length} liquidation targets`);
  if (targets.length === 0) {
    runtime2.log("No liquidation targets found. Skipping transaction.");
    runtime2.log("=== Liquidation Workflow Completed ===");
    return "";
  }
  const liquidationTargets = targets.map((t) => ({
  ...
  runtime2.log("=== Liquidation Workflow Completed ===");
  runtime2.log(`Targets processed: ${targets.length} | TxHash: ${txHash}`);
  return txHash;
};
var onCronTrigger = (runtime2, payload) => {
  ...
  runtime2.log("Liquidation workflow triggered by cron schedule");
  return executeLiquidationWorkflow(runtime2);
};
